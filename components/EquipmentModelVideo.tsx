...
export function EquipmentModelVideo({
  youtubeId,
  modelName,
  compact = false,
}: {
  youtubeId: string;
  modelName: string;
  // compact = inline use inside a model card on the vendor page (player only,
  // no big section heading). Default false keeps the standalone section look.
  compact?: boolean;
}) {
    ...
    return (
      <div className="mt-4">
        <p className="text-xs font-medium text-gray-500 mb-2 flex items-center gap-1.5">
          <Youtube className="w-4 h-4 text-[#22C55E]" />
