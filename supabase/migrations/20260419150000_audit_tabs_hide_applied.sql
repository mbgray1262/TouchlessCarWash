-- Photo-audit tabs were showing already-applied results, so approving a listing
-- didn't decrement the count. Each tab should show pending work only: items
-- where the AI made a suggestion that has NOT yet been applied.
--
-- Fix both RPCs:
--   - get_photo_audit_stats: equipment_total / heroes_total / cleanup_total now
--     exclude applied rows.
--   - get_photo_audit_page: equipment / heroes / cleanup filters now exclude
--     applied rows too, so approve-and-next actually shrinks the queue.
...
AS $$
  ...
  SELECT json_build_object(
    ...
    -- Pending-only totals: count decrements as admin approves/rejects
    'equipment_total', COUNT(*) FILTER (WHERE equipment_brand IS NOT NULL AND NOT applied),
    'heroes_total', COUNT(*) FILTER (WHERE hero_quality = 'poor' AND suggested_hero_url IS NOT NULL AND NOT applied),
    'cleanup_total', COUNT(*) FILTER (WHERE array_length(photos_to_remove, 1) > 0 AND NOT applied)
  )
    ...
    WHERE
      CASE p_filter
        WHEN 'review' THEN
          ...
          (hero_quality = 'poor' AND suggested_hero_url IS NOT NULL AND NOT applied) OR
          (array_length(photos_to_remove, 1) > 0 AND NOT applied)
        -- Each tab now hides applied rows so approving an item removes it.
        WHEN 'equipment' THEN equipment_brand IS NOT NULL AND NOT applied
        WHEN 'heroes' THEN hero_quality = 'poor' AND suggested_hero_url IS NOT NULL AND NOT applied
        WHEN 'cleanup' THEN array_length(photos_to_remove, 1) > 0 AND NOT applied
        ELSE true
      END
