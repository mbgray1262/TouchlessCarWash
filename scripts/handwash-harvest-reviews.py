...
(source='gmaps-handwash', is_hand_wash_evidence=true).
...
LOG = os.path.join(SCRIPT_DIR, 'handwash-harvest-reviews.log')
...
# specific enough that "hand" idioms ("hands down", "handicap", "on the other hand")
# don't match, so no idiom blocklist is needed.
STRONG_RE = re.compile(
    r'\bhand[\s-]?wash\w*|'                                   # hand wash / handwashed / hand-washing
    r'\bwash\w*\b[^.!?]{0,30}?\bby\s+hand\b|'                 # "washed my car by hand"
    r'\bhand[\s-]?dr(?:y|ies|ied|ying)\b|'                    # hand dry / hand dried
    r'\bhand[\s-]?(?:wax|buff|polish)\w*|'                    # hand wax / hand buffed
    r'\bdetail(?:ing|ed|er|ers)\b|'                           # detailing / detailed / detailer (service form)
...
WEAK_RE = re.compile(
    ...
    re.I)
# NEGATIVE — reject the whole review: it describes a DIY / self-serve / machine wash,
# which contradicts an ATTENDED hand wash. (Deliberately NOT rejecting on "automatic" —
# hand washes are often praised in comparison to automatics, e.g. "automatic was down so
# they did it by hand".)
NEG_RE = re.compile(
    r'\bself[\s-]?serv\w*|'
...
def stamp_mined(lid):
    ...
    try:
        sb('PATCH', f'/rest/v1/listings?id=eq.{lid}',
           body={'hand_wash_reviews_mined_at': datetime.datetime.now(datetime.timezone.utc).isoformat()})
...
async def harvest_one(page, listing, stats):
    ...
    for r in reviews:
        ...
        rows.append({
            ...
            'review_id': f"hw-{lid}-{r['review_id']}", 'source': SOURCE,
            'is_hand_wash_evidence': True, 'hand_wash_keywords': hits,
...
async def main():
    ...
    if discover:
        ...
        for off in range(0, 40000, 1000):
            rows = sb('GET', f'/rest/v1/listings?select=id,name,google_place_id&is_hand_wash=is.null'
        ...
        while True:
            rows = sb('GET', f'/rest/v1/listings?select=id,name,google_place_id'
                              f'&is_hand_wash=eq.true'
                              f'&hand_wash_reviews_mined_at=is.null'
