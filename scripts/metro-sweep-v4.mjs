#!/usr/bin/env node
/**
 * Metro sweep v4 — burns remaining SerpAPI credits with rate-limit respect.
 *
 * - Tunnel-chain blocklist applied at promotion time (no post-revert)
 * - Sleeps between queries to stay under 200/hour SerpAPI cap
 * - Retries on throttle with exponential backoff
 * - Covers metros skipped in v3 due to throttle + additional gap metros
 */
...
// Metros skipped in v3 (throttle) + next-tier gap metros. Will consume
// remaining credits (~31 as of v3 end). Script stops if it runs out of
// credits or completes all metros.
const METROS = [
  // Thrown out of v3 by throttle
  ['Birmingham', 'AL'], ['Huntsville', 'AL'], ['Mobile', 'AL'], ['Montgomery', 'AL'],
  ['Portland', 'ME'], ['Bangor', 'ME'],
  ['Boise', 'ID'], ['Nampa', 'ID'],
  ['Kansas City', 'KS'],
  // Additional gap metros (mid-size Southern/Midwestern)
  ['New Orleans', 'LA'], ['Baton Rouge', 'LA'], ['Shreveport', 'LA'],
  ['Jackson', 'TN'], ['Chattanooga', 'TN'], ['Clarksville', 'TN'],
  ['Charleston', 'WV'], ['Huntington', 'WV'], ['Morgantown', 'WV'],
  ['Manchester', 'NH'], ['Nashua', 'NH'], ['Concord', 'NH'],
  ['Burlington', 'VT'], ['Montpelier', 'VT'],
  ['Billings', 'MT'], ['Bozeman', 'MT'], ['Missoula', 'MT'],
  ['Fargo', 'ND'], ['Bismarck', 'ND'],
  ['Sioux Falls', 'SD'], ['Rapid City', 'SD'],
];

const TUNNEL_DOMAINS = new Set([
  'tidalwaveautospa.com', 'whistleexpresscarwash.com', 'take5carwashes.com',
  'tsunamiexpress.com', 'mistercarwash.com', 'quickquack.com',
  'tommysexpress.com', 'zipscarwash.com', 'whitewaterexpress.com',
  'rocketwashwi.com', 'americanpridexpress.com', 'myexpresscarwash.com',
  'quicknclean.net', 'xpressolube.com', 'way.com',
]);
...
const SLEEP_MS = 22_000; // ~160/hour, safely under 200/hour
const sleep = ms => new Promise(r => setTimeout(r, ms));

async function serpSearch(q, retries = 3) {
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    if (json.error && /throttle|rate/i.test(json.error)) {
      const wait = 60_000 * (attempt + 1);
      console.log(`  ⏸  throttled, sleeping ${wait/1000}s...`);
      await sleep(wait);
      continue;
    }
    return json;
...
for (const [city, state] of METROS) {
  ...
  creditsUsed++;
  if (json.error) {
    console.log(`${city}, ${state}: ${json.error}`);
  ...
  console.log(`${String(city+', '+state).padEnd(24)} results:${String(results.length).padStart(3)}  promoted:${matchedHere}  blocked:${blockedHere}  credits:${creditsRemaining}`);
  await sleep(SLEEP_MS);
}
...
if (promotions.size > 0) {
  ...
  for (let i = 0; i < ids.length; i += 200) {
    ...
    const { error } = await sb.from('listings').update({
      ...
      classification_source: 'promoted_apr16_metro_sweep_v4',
      crawl_notes: 'Promoted: metro-sweep v4 domain match with tunnel-chain blocklist. Held at is_approved=false pending enrichment.',
    }).in('id', batch);
...
writeFileSync('scripts/discovery-output/metro-sweep-v4.json', JSON.stringify({
  creditsUsed, creditsRemaining, promotions: promotions.size,
