...
async def main():
    ...
    off=0; listings=[]
    while True:
        rows=sb_get(f'/rest/v1/listings?select=id,name,address,city,state,zip&is_touchless=eq.true&is_approved=eq.true&google_place_id=is.null&limit=1000&offset={off}')
        if not rows: break
