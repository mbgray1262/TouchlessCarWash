...
import { HAND_WASH_LIVE, publicHandWashListings, MIN_HAND_WASH_CITY } from '@/lib/hand-wash';
...
// Pre-render all states; those with no public hand-wash listings 404 at runtime.
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Hand Car Washes in ${name}${n ? ` — ${n} Locations` : ''} | ${year}`;
  const description = `Find hand car washes in ${name} — ${n} verified full-service hand washing and detailing locations where attendants wash your car by hand. Hours, ratings, and directions.`;
  return {
...
export default async function HandWashStatePage({ params }: Props) {
  const code = codeFromSlug(params.state);
  if (!code) notFound();
  const name = getStateName(code);
  const listings = await getStateListings(code);
  // A state page with no public hand-wash listings is thin — 404 it (and it
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <h1 className="text-3xl font-extrabold text-[#0F2744]">Hand Car Washes in {name}</h1>
        <p className="mt-2 text-gray-600">
          {listings.length} verified hand car wash {listings.length === 1 ? 'location' : 'locations'} across {cities.length} {cities.length === 1 ? 'city' : 'cities'} — full-service hand washing and detailing where attendants clean your car by hand.
        </p>
        <Link href="/best-hand-wash" className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-[#22C55E] hover:underline">
          🏆 See the best-rated hand car washes by metro →
        </Link>
        ...
        {cities.map(([city, group]) => (
          <section key={city} className="mt-10">
            <h2 className="text-xl font-bold text-[#0F2744] mb-4 flex items-center gap-2">
              ...
              {group.length >= MIN_HAND_WASH_CITY
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {group.map(l => <ListingCard key={l.id} listing={l} context="hand-wash" />)}
            </div>
          </section>
        ))}
      </div>
