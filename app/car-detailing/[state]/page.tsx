...
import { DETAILING_LIVE, publicDetailingListings, MIN_DETAILING_CITY } from '@/lib/detailing';
...
// Pre-render all states; those with no public detailing listings 404 at runtime.
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Auto Detailing in ${name}${n ? ` — ${n} Detailers` : ''} | ${year}`;
  const description = `Find auto detailers in ${name} — ${n} verified detailers offering paint correction, ceramic coating, PPF, and full interior detailing. Scored on real reviews, with hours, ratings, and directions.`;
  return {
...
export default async function DetailingStatePage({ params }: Props) {
  const code = codeFromSlug(params.state);
  if (!code) notFound();
  const name = getStateName(code);
  const listings = await getStateListings(code);
  // A state page with no public detailing listings is thin — 404 it (and it
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <h1 className="text-3xl font-extrabold text-[#0F2744]">Auto Detailing in {name}</h1>
        <p className="mt-2 text-gray-600">
          {listings.length} verified auto {listings.length === 1 ? 'detailer' : 'detailers'} across {cities.length} {cities.length === 1 ? 'city' : 'cities'} — paint correction, ceramic coating, paint protection film, and full interior detailing.
        </p>
        <Link href="/best-detailing" className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-[#22C55E] hover:underline">
          🏆 See the best-rated auto detailers by metro →
        </Link>
        ...
        {cities.map(([city, group]) => (
          <section key={city} className="mt-10">
            <h2 className="text-xl font-bold text-[#0F2744] mb-4 flex items-center gap-2">
              ...
              {group.length >= MIN_DETAILING_CITY
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {group.map(l => <ListingCard key={l.id} listing={l} context="detailing" />)}
            </div>
          </section>
        ))}
      </div>
