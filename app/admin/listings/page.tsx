...
export default function AdminListingsPage() {
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to fetch listings';
      setError(errorMsg);
  ...
  const filterListings = () => {
    ...
    if (chainFilter !== 'all') {
      if (chainFilter === 'chains_only') {
        filtered = filtered.filter((l) => !!l.parent_chain);
      } else if (chainFilter === 'independent') {
        filtered = filtered.filter((l) => !l.parent_chain);
      } else if (chainFilter === 'missing_location_url') {
        filtered = filtered.filter((l) => !!l.parent_chain && !l.location_page_url);
      } else {
        filtered = filtered.filter((l) => l.parent_chain === chainFilter);
      }
    }

    if (statusFilter !== 'all') {
      if (statusFilter === 'pending') {
        filtered = filtered.filter((l) => (l.crawl_status === 'pending' || l.crawl_status === null) && l.website);
      } else if (statusFilter === 'crawled') {
        filtered = filtered.filter((l) => l.crawl_status === 'crawled');
      } else if (statusFilter === 'failed') {
        filtered = filtered.filter((l) => l.crawl_status === 'failed');
      } else if (statusFilter === 'no_website') {
        filtered = filtered.filter((l) => l.crawl_status === 'no_website' || !l.website);
      } else if (statusFilter === 'touchless') {
        filtered = filtered.filter((l) => l.is_touchless === true);
      } else if (statusFilter === 'not_touchless') {
        filtered = filtered.filter((l) => l.is_touchless === false);
      }
    }
    ...
    filtered.sort((a, b) => {
      ...
      if (sortField === 'last_crawled_at') {
        aVal = a.last_crawled_at ?? '';
        bVal = b.last_crawled_at ?? '';
      } else {
  ...
  const verifyBatch = async () => {
    const pendingListings = listings.filter(
      (l) => (l.crawl_status === 'pending' || l.crawl_status === null) && l.website
    );
    ...
    const effectiveBatchSize = Math.min(pendingListings.length, batchSize);
    ...
    const initialResults: BatchVerifyResult[] = pendingListings.slice(0, effectiveBatchSize).map((l) => ({
      id: l.id,
      name: l.name,
      website: l.website!,
    ...
    try {
      for (let i = 0; i < effectiveBatchSize; i++) {
        ...
        try {
          ...
          // Step 2: Extract amenities & photos (touchless only)
  ...
  const stats = {
    ...
    pending: listings.filter((l) => (l.crawl_status === 'pending' || l.crawl_status === null) && l.website).length,
    verified: listings.filter((l) => l.crawl_status === 'crawled').length,
    failed: listings.filter((l) => l.crawl_status === 'failed').length,
    touchless: listings.filter((l) => l.is_touchless === true).length,
    noWebsite: listings.filter((l) => !l.website || l.crawl_status === 'no_website').length,
    featured: listings.filter((l) => l.is_featured === true).length,
  };

  const statusFilteredListings = listings.filter((l) => {
    if (statusFilter === 'pending') return (l.crawl_status === 'pending' || l.crawl_status === null) && !!l.website;
    if (statusFilter === 'crawled') return l.crawl_status === 'crawled';
    if (statusFilter === 'failed') return l.crawl_status === 'failed';
    if (statusFilter === 'no_website') return l.crawl_status === 'no_website' || !l.website;
    if (statusFilter === 'touchless') return l.is_touchless === true;
    if (statusFilter === 'not_touchless') return l.is_touchless === false;
    return true;
  });

  const chainNames = Array.from(
    new Set(statusFilteredListings.map((l) => l.parent_chain).filter((c): c is string => !!c))
  ).sort();

  const chainLocationsCount = chainNames.reduce<Record<string, number>>((acc, chain) => {
    acc[chain] = statusFilteredListings.filter((l) => l.parent_chain === chain).length;
    return acc;
  }, {});

  const chainsMissingLocationUrl = statusFilteredListings.filter((l) => !!l.parent_chain && !l.location_page_url).length;
      ...
      <div className="container mx-auto px-4 py-8">
        {error && (
          <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
            ...
            <Button onClick={fetchListings} className="mt-3" size="sm">Try Again</Button>
          </div>
        )}

        {chainsMissingLocationUrl > 0 && (
          <div className="mb-4 flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-amber-800">
                {chainsMissingLocationUrl} chain location{chainsMissingLocationUrl !== 1 ? 's' : ''} missing a location-specific URL
              </p>
        ...
        <div className="mb-6">
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                onClick={verifyBatch}
                disabled={batchVerifying || stats.pending === 0}
                className="bg-[#22C55E] hover:bg-[#16A34A] text-white"
              >
                {batchVerifying ? (
                  <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Verifying...</>
                ) : (
                  `Verify Next ${Math.min(stats.pending, batchSize)}`
                )}
              </Button>
        ...
        <div className="grid grid-cols-8 gap-4 mb-6">
          ...
          ><CardContent className="pt-6"><div className={`text-2xl font-bold text-[#0F2744]`}>{stats.total}</div><div className="text-sm text-gray-600">Total Listings</div>{statusFilter === 'all' && chainFilter === 'all' && !featuredFilter && <div className="mt-2 h-0.5 bg-[#0F2744] rounded-full" />}</CardContent></Card>
          <Card
          ...
          ><CardContent className="pt-6"><div className="text-2xl font-bold text-yellow-600">{stats.pending}</div><div className="text-sm text-gray-600">Pending Verification</div>{statusFilter === 'pending' && <div className="mt-2 h-0.5 bg-yellow-400 rounded-full" />}</CardContent></Card>
          <Card
            className="cursor-pointer hover:border-blue-400 transition-colors"
            onClick={() => { setStatusFilter(statusFilter === 'crawled' ? 'all' : 'crawled'); setChainFilter('all'); }}
          ><CardContent className="pt-6"><div className="text-2xl font-bold text-blue-600">{stats.verified}</div><div className="text-sm text-gray-600">Verified</div>{statusFilter === 'crawled' && <div className="mt-2 h-0.5 bg-blue-400 rounded-full" />}</CardContent></Card>
          <Card
          ...
          ><CardContent className="pt-6"><div className="text-2xl font-bold text-red-500">{stats.failed}</div><div className="text-sm text-gray-600">Failed</div>{statusFilter === 'failed' && <div className="mt-2 h-0.5 bg-red-400 rounded-full" />}</CardContent></Card>
          <Card
          ...
          ><CardContent className="pt-6"><div className="text-2xl font-bold text-[#22C55E]">{stats.touchless}</div><div className="text-sm text-gray-600">Touchless</div>{statusFilter === 'touchless' && <div className="mt-2 h-0.5 bg-[#22C55E] rounded-full" />}</CardContent></Card>
          <Card
          ...
          ><CardContent className="pt-6"><div className="text-2xl font-bold text-gray-600">{stats.noWebsite}</div><div className="text-sm text-gray-600">No Website</div>{statusFilter === 'no_website' && <div className="mt-2 h-0.5 bg-gray-400 rounded-full" />}</CardContent></Card>
          <Card
            className="cursor-pointer hover:border-[#0F2744] transition-colors"
            onClick={() => { setChainFilter(chainFilter === 'chains_only' ? 'all' : 'chains_only'); setStatusFilter('all'); }}
          >
            <CardContent className="pt-6">
              <div className="text-2xl font-bold text-[#0F2744]">{chainNames.length}</div>
              <div className="text-sm text-gray-600">Chains</div>
              {chainsMissingLocationUrl > 0 && (
                <div className="flex items-center gap-1 mt-1">
                  <TriangleAlert className="w-3 h-3 text-amber-500" />
                  <span className="text-xs text-amber-600">{chainsMissingLocationUrl} need URL</span>
                </div>
              )}
              ...
              >
                ...
                {chainNames.map((chain) => (
                  <option key={chain} value={chain}>
                    {chain} ({chainLocationsCount[chain]})
                  </option>
                ))}
              </select>
            </div>
