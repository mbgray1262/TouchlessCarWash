...
export function usePhotoAudit() {
  ...
  const [washType, setWashType] = useState<'touchless' | 'self_serve' | 'hand_wash' | 'detailing'>('touchless');
  const washColRef = useRef<'is_touchless' | 'is_self_service' | 'is_hand_wash' | 'is_detailing'>('is_touchless');
  washColRef.current = washType === 'self_serve' ? 'is_self_service' : washType === 'hand_wash' ? 'is_hand_wash' : washType === 'detailing' ? 'is_detailing' : 'is_touchless';
  ...
  const reviewedColRef = useRef<'photo_audited_at' | 'self_service_reviewed_at' | 'hand_wash_reviewed_at' | 'detailing_reviewed_at'>('photo_audited_at');
  reviewedColRef.current = washType === 'self_serve' ? 'self_service_reviewed_at' : washType === 'hand_wash' ? 'hand_wash_reviewed_at' : washType === 'detailing' ? 'detailing_reviewed_at' : 'photo_audited_at';
  ...
  const loadPage = useCallback(async (filter: ViewFilter, pageNum: number, unreviewed: boolean = false) => {
    ...
    if (filter === 'all') {
      ...
      const grouped = selfServe || washColRef.current === 'is_hand_wash' || washColRef.current === 'is_detailing';
      if (grouped) {
