...
import { Search, Plus, Pencil, Trash2, ExternalLink, Building2, Link2, Globe, ChevronDown, ChevronUp, X, Check, Loader2 } from 'lucide-react';
...
export default function AdminVendorsPage() {
  ...
  const [sortField, setSortField] = useState<'canonical_name' | 'updated_at' | 'listing_count'>('updated_at');
  ...
  const filterAndSort = () => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(
        (v) =>
          ...
          (v.domain && v.domain.toLowerCase().includes(q)) ||
    ...
    result.sort((a, b) => {
      ...
      } else if (sortField === 'listing_count') {
        valA = a.listing_count ?? 0;
        valB = b.listing_count ?? 0;
      } else {
        valA = a.updated_at;
        valB = b.updated_at;
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingVendor) {
        ...
        if (error) throw error;
        toast({ title: 'Saved', description: `${payload.canonical_name} updated successfully` });
      } else {
        const { error } = await supabase.from('vendors').insert(payload);
        if (error) throw error;
        toast({ title: 'Created', description: `${payload.canonical_name} added successfully` });
      }
  ...
  const handleDelete = async (vendor: Vendor) => {
    if (!confirm(`Delete "${vendor.canonical_name}"? This cannot be undone.`)) return;
    setDeletingId(vendor.id);
    try {
      const { error } = await supabase.from('vendors').delete().eq('id', vendor.id);
      if (error) throw error;
      toast({ title: 'Deleted', description: `${vendor.canonical_name} removed` });
      fetchVendors();
    } catch (err) {
  ...
  const totalListings = vendors.reduce((sum, v) => sum + (v.listing_count ?? 0), 0);
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button onClick={openCreate} className="bg-[#0F2744] hover:bg-[#1a3a5c] text-white gap-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <Card>
            <CardContent className="pt-4 pb-4">
              ...
              <div className="text-sm text-gray-500 mt-0.5">Total Vendors</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 pb-4">
              <div className="text-2xl font-bold text-gray-900">{chainCount}</div>
              <div className="text-sm text-gray-500 mt-0.5">Chains</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 pb-4">
              <div className="text-2xl font-bold text-gray-900">{standaloneCount}</div>
              <div className="text-sm text-gray-500 mt-0.5">Standalone</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 pb-4">
              <div className="text-2xl font-bold text-gray-900">{totalListings}</div>
              <div className="text-sm text-gray-500 mt-0.5">Linked Listings</div>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent className="pt-4 pb-4">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder="Search vendors by name, domain, or description..."
                  value={searchQuery}
                ...
                <thead>
                  <tr className="border-b bg-gray-50">
                    ...
                    <th className="text-left px-4 py-3 font-medium text-gray-600">
                      <button
                        className="flex items-center gap-1 hover:text-gray-900"
                        onClick={() => toggleSort('listing_count')}
                      >
                        Listings <SortIcon field="listing_count" />
                      </button>
                    </th>
                ...
                <tbody>
                  {loading ? (
                      ...
                      <td colSpan={6} className="text-center py-12 text-gray-400">
                        {vendors.length === 0 ? (
                          <div>
                            ...
                            <p>No vendors yet.</p>
                            <p className="text-xs mt-1">Click &ldquo;Add Vendor&rdquo; to get started.</p>
                          </div>
                        ) : (
                    ...
                    filteredVendors.map((vendor) => (
                      <tr key={vendor.id} className="border-b last:border-0 hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            {vendor.logo_url ? (
                              <img
                                src={vendor.logo_url}
                                alt={vendor.canonical_name}
                                className="w-8 h-8 rounded object-contain bg-gray-100 border flex-shrink-0"
                              />
                            ) : (
                            ...
                            <div>
                              <div className="font-medium text-gray-900">{vendor.canonical_name}</div>
                              {vendor.description && (
                                <div className="text-xs text-gray-500 line-clamp-1 max-w-xs">{vendor.description}</div>
                              )}
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3 hidden lg:table-cell">
                          <Badge
                            variant="outline"
                            className={vendor.is_chain ? 'border-blue-200 text-blue-700 bg-blue-50' : 'border-gray-200 text-gray-600'}
                          >
                            {vendor.is_chain ? 'Chain' : 'Standalone'}
                          </Badge>
                        </td>
                        <td className="px-4 py-3">
                          <span className="font-medium text-gray-900">{vendor.listing_count ?? 0}</span>
                        </td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg">
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <Input
                  placeholder="e.g. Mister Car Wash"
                  value={formData.canonical_name}
                  onChange={(e) => setFormData((f) => ({ ...f, canonical_name: e.target.value }))}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Domain</label>
                <Input
                  placeholder="e.g. mistercarwash.com"
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
                    formData.is_chain ? 'bg-[#0F2744]' : 'bg-gray-200'
                  }`}
                >
                  <span
                    className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform ${
                      formData.is_chain ? 'translate-x-4' : 'translate-x-1'
                    }`}
                  />
                </button>
